...
class EventAdapter(private var events: List<Event>, private val clickListener: (Event) -> Unit) : RecyclerView.Adapter<EventAdapter.EventViewHolder>() {
    ...
    class EventViewHolder(private val binding: ItemEventBinding) : RecyclerView.ViewHolder(binding.root) {
        ...
        private fun getFormattedDate(time: Long?): String {
            ...
            return when {
                today.get(Calendar.YEAR) == eventDate.get(Calendar.YEAR) && today.get(Calendar.DAY_OF_YEAR) == eventDate.get(Calendar.DAY_OF_YEAR) -> {
                    "Сегодня"
                }
                today.get(Calendar.YEAR) == eventDate.get(Calendar.YEAR) && today.get(Calendar.DAY_OF_YEAR) + 1 == eventDate.get(Calendar.DAY_OF_YEAR) -> {
                    "Завтра"
                }
                else -> {
